...
public class BinarySearchTree {
	...
	private Node get(Node ptr, int key) {
		if (ptr == null)
			return null;
		if (ptr.key < key)
			return get(ptr.rightChild, key);
		else if (ptr.key > key)
			return get(ptr.leftChild, key);
		else
			return ptr;
	}
	...
	private Node delete(Node ptr, int key) {
		if (ptr == null)
			return ptr;
		if (ptr.key < key)
			return ptr.rightChild = delete(ptr.rightChild, key);
		else if (ptr.key > key)
			return ptr.leftChild = delete(ptr.leftChild, key);
		...
		while (ret.size() == level + 1) {
			...
			for (Node node : que) {
				if (node.leftChild != null)
					add.add(node.leftChild);
				if (node.rightChild != null)
					add.add(node.rightChild);
			}
			if (add.size() != 0)
				ret.add(add);
			level++;
