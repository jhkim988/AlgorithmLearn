...
public class BOJ20176 {
  public static void main(String[] args) throws IOException {
    BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
    long[] p = new long[60_001];
    int[] q;
    long[] r = new long[60_001];

    int pnum = Integer.parseInt(br.readLine());
    StringTokenizer st = new StringTokenizer(br.readLine());
    for (int i = 0; i < pnum; i++) {
      p[Integer.parseInt(st.nextToken()) + 30_000] = 1;
    }
    ...
    for (int i = 0; i < rnum; i++) {
      r[Integer.parseInt(st.nextToken()) + 30_000] = 1;
    }

    long[] product = product(p, r);
    int num = 0;
    for (int i = 0; i < qnum; i++) {
      num += product[(q[i] + 30_000)*2];
    }
