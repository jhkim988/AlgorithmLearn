...
public class BOJ10538 {
  public static void main(String[] args) throws IOException {
    ...
    int j = 0;
    for (int i = 1; i < wp; i++) {
      while (j > 0 && !equal(pat, 0, i, pat, 0, j, hp)) j = pi[j-1];
      if (equal(pat, 0, i, pat, 0, j, hp)) pi[i] = ++j;
    }
  ...
  static boolean equal(char[][] a, int arow, int acol, char[][] b, int brow, int bcol, int n) {
    for (int i = 0; i < n; i++) {
