...
public class BOJ1006 {
  public static void main(String[] args) throws IOException {
    ...
    while (t-- > 0) {
      ...
      dp[3][0][0] = (input[0][n-1]+input[0][0] <= w && input[1][n-1]+input[1][0] <= w) ? 2 : 0;

      dp[0][1][1] = input[0][0]+input[0][1] <= w ? 1 : 0;
      dp[0][2][1] = input[1][0]+input[1][1] <= w ? 1 : 0;
      dp[0][0][1] = Integer.max(dp[0][0][1], dp[0][0][0]+(input[0][1]+input[1][1] <= w ? 1 : 0));
      dp[0][0][1] = Integer.max(dp[0][0][1], (input[0][0]+input[0][1] <= w && input[1][0]+input[1][1] <= w) ? 2 : 0);

      for (int j = 0; j < 3; j++) {
        dp[1][0][1] = Integer.max(dp[1][0][1], dp[1][j][0] + (input[0][1]+input[1][1] <= w ? 1 : 0));
        dp[2][0][1] = Integer.max(dp[2][0][1], dp[2][j][0] + (input[0][1]+input[1][1] <= w ? 1 : 0));
      }

      dp[1][2][1] = dp[1][1][0] + (input[1][0]+input[1][1] <= w ? 1 : 0);
      dp[2][1][1] = dp[2][2][0] + (input[0][0]+input[0][1] <= w ? 1 : 0);
      
      for (int k = 0; k < 4; k++) {
        for (int i = 2; i < n; i++) {
          if (input[0][i]+input[1][i] <= w) {
            for (int j = 0; j < 3; j++) {
              dp[k][0][i] = Integer.max(dp[k][j][i], dp[k][j][i-1]+1);
            }
          }
