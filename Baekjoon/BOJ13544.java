...
public class BOJ13544 {
  private static class MergeSortTree {
    ...
    int query(String q, int lastAns) {
      StringTokenizer st = new StringTokenizer(q);
      int left = Integer.parseInt(st.nextToken()) ^ lastAns;
      int right = Integer.parseInt(st.nextToken()) ^ lastAns;
  ...
  public static void main(String[] args) throws IOException {
    ...
    int lastAns = 0;
    while (q-- > 0) {
      lastAns = tree.query(br.readLine(), lastAns);
      bw.write(Integer.toString(lastAns));
      bw.newLine();
    }
    bw.flush();
  }
